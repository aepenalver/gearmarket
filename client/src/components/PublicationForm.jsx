...
function PublicationForm({ onSubmit, submitting }) {
  const {
    register,
    handleSubmit,
    formState: { errors },
      ...
      <div className="card-body p-4">
        <div className="row g-3">
          ...
          <div className="col-md-6">
            ...
            />
            {errors.image_url ? (
              <small className="text-danger">{errors.image_url.message}</small>
            ) : null}
          </div>
          <div className="col-md-3">
            <label className="form-label fw-semibold">Categoría</label>
            <select className="form-select" {...register('category')}>
              {mockCategories.filter((category) => category !== 'todos').map((category) => (
